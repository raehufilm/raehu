...
`index.html` is now generated by the same `pages/works` generator that produces the work pages. It combines the landing-page hero with generated root works grids at `#works`. The archived pre-generator version is kept as `index.backup.html`.
...
- Root works grids generated from valid `pages/works/films/` and `pages/works/commercials/` content
...
- **2026-06-02** — Backed up the previous landing page as `index.backup.html` and moved generated works discovery into the root homepage. `scripts/generate_pages.py` now generates `index.html`, the `/works/` redirect, and each work detail page in one pass; the public works index is `raehu.com/#works`.
- **2026-06-02** — Added 12 generated commercial proof-of-concept pages from Rae's public Vimeo profile, excluding the GUGU/Strange Fruit and Champion entries that already had pages. Each new commercial lives under `pages/works/commercials/<slug>/`, uses its Vimeo trailer link, and has non-overlapping stills from `01 Stills` for note, highlight, and BTS sections.
