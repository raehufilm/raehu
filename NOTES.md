...
- **2026-06-01** — Added the `rae hu 樂瑞` brand link to the top-left of generated work-page headers, matching the landing page header treatment while keeping `works`, `about`, and `contact` on the top right.
- **2026-06-02** — Reorganized generated work-page source content into `pages/works/commercials/` and `pages/works/films/`. The generator now traverses both categories, requires slugs to be unique across them, and still publishes each valid work at the public URL shape `raehu.com/works/<slug>/`.
- **2026-06-02** — Added the generated works index page at `works/index.html`, served as `raehu.com/works/`. It uses the same left-side section tracker behavior as generated work pages, stacks `films` and `commercials` irregular grids, and creates each grid entry automatically from valid category content using the work's Vimeo trailer poster image.
